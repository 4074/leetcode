...
var distanceBetweenBusStops = function(distance, start, destination) {
    var one = 0, two = 0, startOne = start, startTwo = start
    while (startOne !== destination) {
        one += distance[startOne]
        startOne += 1
        if (startOne >= distance.length) startOne = 0
    }
    
    while (startTwo !== destination) {
        startTwo -= 1
        if (startTwo < 0) startTwo = distance.length - 1
        two += distance[startTwo]
        
    }
    
